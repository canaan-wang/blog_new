---
title: "Go 语言垃圾回收（GC）全面解析"
date: "2025-12-28"
summary: "Go 的垃圾回收（Garbage Collection，GC）是其核心特性之一，旨在自动管理内存、避免内存泄漏，让开发者无需手动分配/释放内存（如 C/C++ 的 `malloc`/`free`）。Go GC 经历了多代演进（从 v1.1 的简单标记清除到 v1.5+ 的并发三色标记），目前已实现 `低延迟`、`高并发` 的垃圾回收，适配高吞吐的服务端场景。"
tags: ["golang", "GC", "垃圾回收"]
category: "golang"
